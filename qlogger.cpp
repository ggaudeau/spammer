...
namespace QLogger
{
	void QLog_Trace(const Qt::HANDLE handle, const QString &message)
	{
        (void)handle;
        (void)message;
		Q_ASSERT(QLog_Trace(QLoggerManager::getInstance()->getModuleName(handle), message));
	}
	bool QLog_Trace(const QString &module, const QString &message)
	{
		QLog_(module, TraceLevel, message);
		return true;
	}

	void QLog_Debug(const Qt::HANDLE handle, const QString &message)
	{
        (void)handle;
        (void)message;
		Q_ASSERT(QLog_Debug(QLoggerManager::getInstance()->getModuleName(handle), message));
	}
	bool QLog_Debug(const QString &module, const QString &message)
	{
		QLog_(module, DebugLevel, message);
		return true;
	}

	void QLog_Info(const Qt::HANDLE handle, const QString &message)
	{
        (void)handle;
        (void)message;
		Q_ASSERT(QLog_Info(QLoggerManager::getInstance()->getModuleName(handle), message));
	}
	bool QLog_Info(const QString &module, const QString &message)
	{
		QLog_(module, InfoLevel, message);
		return true;
	}

	void QLog_Warning(const Qt::HANDLE handle, const QString &message)
	{
		QLog_Warning(QLoggerManager::getInstance()->getModuleName(handle), message);
	}
	bool QLog_Warning(const QString &module, const QString &message)
	{
		QLog_(module, WarnLevel, message);
		return true;
	}

	void QLog_Error(const Qt::HANDLE handle, const QString &message)
	{
		QLog_Error(QLoggerManager::getInstance()->getModuleName(handle), message);
	}
	bool QLog_Error(const QString &module, const QString &message)
	{
		QLog_(module, ErrorLevel, message);
		return true;
	}

	void QLog_Fatal(const Qt::HANDLE handle, const QString &message)
	{
		QLog_Fatal(QLoggerManager::getInstance()->getModuleName(handle), message);
	}
	bool QLog_Fatal(const QString &module, const QString &message)
	{
	...
	bool QLoggerManager::addDestination(const QString &fileDest, const Qt::HANDLE handle,
		const QString &module, LogLevel level)
	{
		moduleNameMap[handle] = module;
		return addDestination(fileDest, module, level);
	}
	...
	QString QLoggerManager::getModuleName(const Qt::HANDLE handle) const
	{
		QLoggerManager *manager = QLoggerManager::getInstance();
		QMutexLocker(&manager->mutex);
		QMap<Qt::HANDLE, QString>::const_iterator cit;
		if ((cit = moduleNameMap.find(handle)) != moduleNameMap.cend())
			return cit.value();
		return QString();
	}
	...
	void QLoggerWriter::write(const QString &module, LogLevel level, const QString &message)
	{
		...
		if (file.open(QIODevice::ReadWrite | QIODevice::Text | QIODevice::Append)) // QIODevice::Truncate -> not working
		{
			...
			QString logLevel = QLoggerManager::levelToText(/*m_*/level);
			QString text = QString("[%1] [%2] {%3} %4\n").arg(dtFormat).arg(logLevel).arg(module).arg(message);
			out << text;
			file.close();

			qDebug() << qPrintable(text.remove(text.size()-1, 1));
		}
	}
}
